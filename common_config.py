...
def add_ak4jet_config(configuration: Configuration, era: str, profile):
    ...
    - The `tight` working point is used (`jet_id >= 2` in the cumulative 0/2/6
      encoding shared by NanoAOD v9 `Jet_jetId` and the correctionlib jet ID
      producer).
    ...
    configuration.add_config_parameters(
        "global",
        {
            ...
            # `xyh::object_selection::select_jet`. Every path uses the cumulative
            # encoding 0 == fail, 2 == pass(tight) & fail(tightLepVeto),
            ...
            "ak4jet_puid_max_pt": EraModifier(
                {
                    **{
                        _era: run2_puid_max_pt  # 50 GeV on v9 CHS jets, 0.0 on the v15 PUPPI path
                        for _era in ERAS_RUN2
                    },
                    **{
    ...
    configuration.add_config_parameters(
        "global",
        {
            # JME publishes jet ID files for Run 3 only. The Run-2 UL NanoAOD-v15
            # files are generated by the jetid-payloads repository (provenance and
            # checksum in payloads/jetid/README.md), one campaign directory per
            # era, and used on the Run-2 v15 input path; profiles on the legacy
            # v9 files read the Jet_jetId branch and need no file.
            "ak4jet_id_file": EraModifier(
                {
                    ...
                    **(
                        {
                            _era: f"payloads/jetid/Run2-{_era}-UL-NanoAODv15/jetid.json.gz"
                            for _era in ERAS_RUN2
                        }
                        if use_run2_v15
                        else {}
        ...
        configuration.add_config_parameters(
            GLOBAL_SCOPES,
            {
                "ele_es_file": EraModifier(
                    {
                        _era: (
                            "/cvmfs/cms-griddata.cern.ch/cat/metadata/EGM/"
                            f"Run2-{_era}-UL-NanoAODv15/2025-12-05/electronSS_EtDependent.json.gz"
                        )
                        for _era in ERAS_RUN2
                    }
                ),
