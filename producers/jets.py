...
# Jet ID
# - For run 2 NanoAOD v9, the jet ID can just be taken from the corresponding
#   column in nanoAOD.
# - For 2022 and 2023, the jet ID in nanoAOD v12 has a bug which must be
#   adressed.
# - For NanoAOD v15 (2024, 2025, and the Run-2 UL v15 inputs), no jet ID column
#   exists; the ID is evaluated from the per-jet composition branches with a
#   correctionlib file in the JME `jetid.json.gz` layout (`ak4jet_id_file`).
#   JME does not publish that file for 2018 UL; it is generated by the
#   jetid-payloads repository and shipped under payloads/jetid/.
JetIDFromCorrectionlib = Producer(
    name="JetID",
    call="""
...
JetID = {
    ...
    ("2024", "2025"): JetIDFromCorrectionlib,
}
